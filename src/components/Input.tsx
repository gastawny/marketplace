  ...
  return (
    <InputBx length={input.length > 0 ? true : false}>
      ...
      {Boolean(errors[props.name]?.message) && (
        <i className="text-sm font-normal tracking-wide absolute right-0 -top-4 px-4 py-5 not-italic text-gray-300 duration-500 pointer-events-none">
...
const InputBx = styled.div<{ length: boolean }>`
  ...
  input:valid ~ .field {
    ${({ length }) => (length ? tw`-translate-y-4 text-sm` : '')}
  }
